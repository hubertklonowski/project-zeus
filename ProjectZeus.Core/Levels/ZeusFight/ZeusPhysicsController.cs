...
namespace ProjectZeus.Core.Levels.ZeusFight
{
    public class ZeusPhysicsController
    {
        ...
        public void UpdateZeusJump(ref Vector2 zeusPosition, ref Vector2 zeusVelocity, ref bool zeusIsJumping,
            ...
            Vector2 playerSize, Vector2 zeusSize, float groundTop, float deltaTime)
        {
            ...
            {
                zeusPosition = new Vector2(zeusPosition.X, groundTop - zeusSize.Y);
                zeusVelocity = Vector2.Zero;
                zeusIsJumping = false;

                Rectangle zeusRect = new Rectangle((int)zeusPosition.X, (int)zeusPosition.Y, (int)zeusSize.X, (int)zeusSize.Y);
